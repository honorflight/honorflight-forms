...
if(array_key_exists($_GET['sobject_type'], $request)){
  ...
  //Tables
  $safe_objects = $request[$_GET["sobject_type"]];
  ...
  foreach($safe_objects as $safe_object){
    ...
    $query = sprintf("SELECT %s from %s", $fieldlist, ($safe_object));
    $response;
    $error;
    try {
